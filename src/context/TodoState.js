...
const TodoState = (props) => {
    ...
    const onChange=(e)=>{
            ...
            // console.log(input);
            settodo({...todo,[e.target.name]:e.target.value,id:Math.floor(Math.random()*1000)})
        
    }

    //Delete the todo
    const deleteTodo=(id)=>{
        settext(text.filter((value)=>{
            return value.id!==id;
        }))
    ...
    return (
        <todoContext.Provider value={{handleClick,onChange,text,input,deleteTodo,editTodo,cinput,setcinput,updateTodo,handleKey,alertFunc,alert}}>
            {props.children}
